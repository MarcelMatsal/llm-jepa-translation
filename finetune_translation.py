"""Finetuning file from the original LLM-JEPA paper but converted to work for developing rich translations.
...
# =========================== Additions for Translation LLM-JEPA ============================================================

def load_and_prepare_dataset_translations(huggingface_dataset, language_pair, tokenizer, model_name, max_length=2048, debug=0, predictors=0,
                                            regular=False, train_all=False, plain=False, front_pred=False, possibly_reverse=False, split="train"):

    """ Load and prepare translation pairs for LLM-JEPA training.
        
        Each sample consists of two parallel sentences in different languages.
        These are treated as the two JEPA views: user (lang1) and assistant (lang2).
    """      

    dataset = load_dataset(huggingface_dataset, language_pair)
    ...
    def tokenize_examples(examples):
        ...
        text_user = example[lang1]
        text_assistant = example[lang2]

        # Randomly Reverser the order to build richer embeddings
            ...
            text_user, text_assistant = text_assistant, text_user
                ...
                for i in range(1, predictors + 1):
                    if front_pred:
                        ...
                        text_user = text_user + f" <|predictor_{i}|>"
        ...
        user_attention_mask.append(tokenized_user["attention_mask"])
        assistant_input_ids.append(tokenized_assistant["input_ids"])
        assistant_attention_mask.append(tokenized_assistant["attention_mask"])

        if debug >= 3 and torch.cuda.current_device() == 0:
            print(f"[{lang1} → {lang2}]")
            print("User text:", text_user)
        ...
        masked_labels = [[-100] * max_length for _ in range(len(user_input_ids))]
        ...
        if regular:
            ...
            # JEPA two-view training
            return {
                "input_ids": user_input_ids,
                "labels": masked_labels,
                "attention_mask": user_attention_mask,
                "input_ids_user": user_input_ids,
                "labels_user": masked_labels,
                "attention_mask_user": user_attention_mask,
                "input_ids_assistant": assistant_input_ids,
                "labels_assistant": masked_labels,
                "attention_mask_assistant": assistant_attention_mask,
            }
    ...
    if torch.cuda.current_device() == 0:
        print("✅ Finished tokenizing translation pairs")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Fine-tune LLM-JEPA for Translation")
    parser.add_argument("--translation_dataset", type=str, default="wmt19",
                        help="Hugging Face translation dataset name (e.g., wmt19, opus_books)")
    parser.add_argument("--lang_pair", type=str, default="en-de",
                        help="Language pair (e.g., en-de, en-fr)")
    parser.add_argument("--model_name", type=str, default="meta-llama/Llama-3.2-1B-Instruct",
                        help="Model name/path")
    parser.add_argument("--output_dir", type=str, default="./llm-jepa-translation",
    ...
    parser.add_argument("--lbd", type=float, default=0.1, help="Lambda for JEPA (similarity) loss weight")
    parser.add_argument("--gamma", type=float, default=0.0, help="Weight for LM loss (set 0 for pure JEPA)")
    ...
    parser.add_argument("--reverse_pred", action="store_true", help="When set, Use `Lang2` to predict `Lang1`.")
    parser.add_argument("--front_pred", action="store_true", help="When set, Put [Pred] token at the beginning of `Text`.")
    parser.add_argument("--split_seed", type=int, default=42)
    args = parser.parse_args()

    # Validate arguments
    if not args.translation_dataset:
        parser.error("Must provide a dataset to train on")
    
    if not args.lang_pair:
        parser.error("Must provide a language pair from the dataset.")

    if torch.cuda.current_device() == 0:
        print("=== LLM-JEPA Translation Fine-tuning ===")
        print(f"Dataset: {args.translation_dataset} ({args.lang_pair})")
        print(f"Model: {args.model_name}")
        print(f"LoRA: {args.lora}, rank={args.lora_rank}")
        print(f"JEPA loss weight: {args.lbd}")
        print(f"LM loss weight: {args.gamma}")
    ...
    # ---------------- Dataset ----------------
    if torch.cuda.current_device() == 0:
        print("\n2. Loading translation datasets...")

    # Train set
    train_dataset = load_and_prepare_dataset_translations(
        huggingface_dataset=args.translation_dataset,
        ...
        reverse_pred=args.reverse_pred,
        split="train",
    )

    # Eval set
    eval_dataset = load_and_prepare_dataset_translations(
        huggingface_dataset=args.translation_dataset,
        language_pair=args.lang_pair,
        tokenizer=tokenizer,
        model_name=args.model_name,
